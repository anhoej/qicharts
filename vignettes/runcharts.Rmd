...
Non-random variation may present itself in several ways. If the process centre is shifting due to improvement or degradation we may observe _unusually_ long runs of consecutive data points on the same side of the median or that the graph crosses the median _unusually_ few times. The length of the longest run and the number of crossings in a random process are predictable within limits and depend on the total number of data points in the run chart.

- A **shift signal** is present if any run of consecutive data points on the same side of the median is longer than the prediction limit, `round(log2(n) + 3)`^[Mark F. Schilling (2012). The Surprising Predictability of Long Runs. Math. Mag. 85, 141-149]. Data points that fall on the median do not count, they do neither break nor contribute to the run.

- A **crossings signal** is present if the number of times the graph crosses the median is smaller than the prediction limit, `qbinom(0.05, n - 1, 0.5)`^[Zhenmin Chen (2010). A note on the runs test. Model Assisted Statistics and Applications 5, 73-77].
...
Besides providing tick mark labels for the x axis, the `x` argument serves as a subrouping vector. If, for instance, one collects data daily but wishes to aggregate data by week, this can be achieved by using weeks as the subgrouping, `x`, vector.
